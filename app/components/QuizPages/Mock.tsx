...
const Mock = ({ test,prompt }: {test:string,prompt:string}) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>, formData: FormData) => {
    e.preventDefault();
    try {
      const res = await axios.post('http://127.0.0.1:5000/mock', 
        { type: formData.type, questions: formData.questions, level: formData.level ,prompt:formData.prompt},
  ...
  return (
    <form className='w-full h-full relative' autoComplete={'on'} onSubmit={formSubmitHandler} method='post'>
      ...
      <div className="flex-center h-[15%] w-full py-10 mt-5 mb-1">
        ...
        <input type="text" name="" id="" inputMode='numeric' required max={1} value={level} pattern='[0-2]' onChange={(e) => { handleLevel(e) }} className='ml-5 w-20 bg-transparent border-b-2 outline-none flex-center px-1 pl-8' />
      </div>
